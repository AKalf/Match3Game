...
public static class BoardFunctions {
    ...
    public static Color[] availColors = { Color.red, Color.blue, Color.green, Color.cyan, Color.magenta, Color.yellow };
    ...
    public static int ReplaceElement(int collum, int row,
        ...
        Transform[] holders,
        int maxScoreAllowed, int currentMaxScorePossible, int totalCollums, int totalRows,
        ref List<AnimationMessage> playingAnimations, float swappingSpeed) {
        CheckIfElementAtPositionIsNull(collum, row, positions);
        int maxScorePossible = 0;
        positions[collum, row].color = GetColorInScore(collum, row, ref positions, matchedElemPositions, totalCollums, totalRows, maxScoreAllowed, ref maxScorePossible, (currentMaxScorePossible >= maxScoreAllowed - 2) ? true : false);
    ...
    public static Color GetColorInScore(int collum, int row, ref BoardElement[, ] positions, bool[, ] matchedElemPositions, int totalCollums, int totalRows, int maxScoreAllowed, ref int currentScorePossible, bool forceGetMin) {
        ...
        for (int i = startingIndex; i < GetAvailableColors().Length + startingIndex; i++) {
            ...
            Color originalColor = positions[collum, row].color;
            ...
            positions[collum, row].color = GetAvailableColors()[index];
            ...
            positions[collum, row].color = originalColor;
        ...
        if (bestPossibleScoreFound > 0 && !forceGetMin) {
            Debug.Log("### SCORE ### Best score found: " + bestPossibleScoreFound + " for element " + positions[collum, row].transform.parent.GetChild(bestScoreIndex));
            currentScorePossible = bestPossibleScoreFound;
            return GetAvailableColors()[bestScoreIndex];
        }
        else {
            Debug.Log("### SCORE ### Worst score found: " + minimumPossibleScoreFound + " for element " + positions[collum, row].transform.parent.GetChild(bestScoreIndex));
    ...
    public static void ToggleHighlightCell(int collum, int row, BoardElement[, ] positions, bool shouldHighlight, Color highlightColor) {
        if (shouldHighlight) {
            positions[collum, row].transform.GetChild(0).gameObject.GetComponent<UnityEngine.UI.Image>().color = highlightColor;
